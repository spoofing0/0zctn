...
KANAL_HEDEF = "@royalbaccaratfree"  # Düzeltildi
...
# ✅ Tamamlanmamış oyunları takip için
watched_incomplete = {}
trend_history = []
...
async def process_bet(game_num, msg, suits):
    ...
    print(f"[İŞLEM] 🎯 #N{game_num} - {card_count} kart işlendi")
    ...
    for signal_game_num, tracker_info in list(martingale_trackers.items()):
        ...
        if signal_won:
            ...
            if current_step < MAX_MARTINGALE_STEPS:
                ...
                try: 
                    await tracker_info["message_obj"].edit(
                        f"**#N{signal_game_num} - {tracker_info['signal_suit']} | ❌ KAYIP**"
                    )
                except: 
                    pass
                trackers_to_remove.append(signal_game_num)
    ...
    if re.search(r"\(([^)]+)\)", msg.text):
        ...
        if arrow_banker:
            ...
            if suits:
                ...
                print(f"[İŞLEM] 🎯 #N{game_num} tamamlandı (👉 banker tarafında)")
                await process_bet(game_num, msg, suits)
        else:
            # 👉 oyuncu tarafında → bekle
            watched_incomplete[msg.id] = (game_num, msg)
            print(f"[BEKLEME] ⏳ #N{game_num} 👉 oyuncu kart açıyor, bekleniyor...")
    ...
    if msg.id in watched_incomplete:
        ...
        if arrow_banker:
            ...
            if suits:
                ...
                print(f"[DÜZENLEME] ✅ #N{game_num} banker tarafına geçti → sonuç işlendi.")
...
async def help_command(event):
    """Yardım komutu"""
    await event.reply("""ℹ️ **Yardım Menüsü**
...
async def stats_command(event):
    ...
    await event.reply(f"""📊 **Performans İstatistikleri:**
...
📉 Maksimum Ardışık Kayıp: {performance_stats['max_consecutive_losses']}
🕐 Aktif Başlangıç: {performance_stats['active_since']}
...
async def analysis_command(event):
    ...
    await event.reply(f"""📈 **Detaylı Sistem Analizi:**
...
⏳ Bekleyen Oyunlar: {len(watched_incomplete)} oyun
...
async def status_command(event):
    """Durum komutu"""
    await event.reply(f"""🟢 **Bot Çalışıyor**

🎯 Aktif Sinyal: {'✅ Evet' if is_signal_active else '❌ Hayır'}
📊 Takip Edilen Sinyaller: {len(martingale_trackers)} sinyal
⏳ Bekleyen Oyunlar: {len(watched_incomplete)} oyun
🔄 Ardışık Kayıp: {performance_stats['consecutive_losses']}/3""")
...
async def patterns_command(event):
    """Pattern komutu"""
    await event.reply("""🎯 **Aktif Patternler:**
...
if __name__ == "__main__":
    print("=" * 60)
    print("🤖 Royal Baccarat Bot Başlatılıyor...")
    print("=" * 60)
    print(f"📡 Kaynak Kanal ID: {KANAL_KAYNAK_ID}")
    print(f"📤 Hedef Kanal: {KANAL_HEDEF}")
    print(f"🎯 Maksimum Martingale Adımı: {MAX_MARTINGALE_STEPS + 1}")
