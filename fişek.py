...
def get_previous_game(current_game, back=10):
...
async def update_martingale(current_game, player_cards_string):
    ...
    for signal_key, info in list(martingale_tracker.items()):
        ...
        if target_suit in player_cards_string:
            ...
            new_text = f"#{bet_game} | {target_suit} - 7D | 10-BC-{type_display} | ✅ {STEP_EMOJIS[current_step]} | {win_message}"
            
            try:
                await client.edit_message(KANAL_HEDEF, info["msg_id"], new_text)
                logger.info(f"✅ #{bet_game} KAZANÇ: Step {current_step}'de kazanıldı")
                info["checked"] = True
                
            except Exception as e:
                logger.error(f"❌ #{bet_game} düzenlenirken hata: {e}")
        else:
            ...
            if next_step > MAX_MARTINGALE_STEP:
                ...
                new_text = f"#{bet_game} | {target_suit} - 7D | 10-BC-{type_display} | ❌ | {loss_message}"
                try:
                    await client.edit_message(KANAL_HEDEF, info["msg_id"], new_text)
                    logger.info(f"❌ #{bet_game} KAYIP: Maksimum step aşıldı")
                    info["checked"] = True
                ...
                new_text = f"#{bet_game} | {target_suit} - 7D | 10-BC-{type_display} | 🔃 {STEP_EMOJIS[next_step]} | {waiting_message}"
                try:
                    await client.edit_message(KANAL_HEDEF, info["msg_id"], new_text)
                    logger.info(f"🔄 #{bet_game} STEP GÜNCELLENDİ: {current_step} → {next_step}")
...
async def send_signal(signal_game, signal_suit, signal_type):
    ...
    # SADECE SİNYAL FORMATI: #419 | ♣️ - 7D | 10-BC-Middle | 🔃 0️⃣ | Devam Ediyor…
    text = f"#{signal_game} | {signal_suit} - 7D | 10-BC-{type_display} | 🔃 {STEP_EMOJIS[0]} | {waiting_message}"
    ...
    try:
        ...
        logger.info(f"🎯 YENİ SİNYAL: #{signal_game} | {signal_suit} - 7D | 10-BC-{type_display}")
        return True
    except Exception as e:
...
async def handler(event):
    ...
    # 2) 10-back stratejisini çalıştır
    try:
        await check_10_game_back_system(game_number, player_cards)
    except Exception as e:
        logger.error(f"❌ 10-back stratejisinde hata: {e}")
...
async def main():
    ...
    startup_msg = (
        ...
        f"🎯 Strateji: 10-back (3 kart pozisyonu)\n"
