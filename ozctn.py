...
performance_stats = {
    ...
    'signal_history': deque(maxlen=1000),  # Son 1000 sinyali sakla
...
# GÜNCELLENMİŞ İSTATİSTİK FONKSİYONLARI
def update_c2_3_stats(c2_3_type, result_type, steps=0):
    """C2-3 tipine göre istatistik günceller"""
...
def get_c2_3_performance():
    """C2-3 tiplerinin performans analizini döndürür"""
...
def get_best_performing_type():
    """En iyi performans gösteren C2-3 tipini döndürür"""
    best_type = None
    best_win_rate = 0
    
    for c2_3_type, data in performance_stats['c2_3_performance'].items():
        stats = data['stats']
        if stats['total'] >= 5:  # Minimum 5 sinyal için değerlendir
            win_rate = (stats['wins'] / stats['total']) * 100
...
def get_worst_performing_type():
    """En kötü performans gösteren C2-3 tipini döndürür"""
    worst_type = None
    worst_win_rate = 100
...
def update_performance_stats(result_type, steps=0, c2_3_type=None):
    ...
    if result_type == 'win':
        ...
        performance_stats['total_profit'] -= (2**steps - 1)  # Martingale kaybı
...
def generate_trend_analysis():
    """Trend analizi raporu oluşturur"""
    if not color_trend:
...
async def handle_eniyi(event):
    """En iyi performans gösteren tip"""
    best_type, best_rate = get_best_performing_type()
    if best_type:
...
async def handle_enkotu(event):
    """En kötü performans gösteren tip"""
    worst_type, worst_rate = get_worst_performing_type()
    if worst_type:
...
async def handle_performans(event):
    """Performans raporu (istatistik ile aynı)"""
    report = generate_performance_report()
    await event.reply(report)

@client.on(events.NewMessage(pattern='(?i)/rapor'))
async def handle_rapor(event):
    """Günlük/haftalık detaylı rapor"""
    daily = get_daily_stats()
    weekly = get_weekly_stats()
    win_rate = calculate_win_rate()
    
    # C2-3 analizini de ekle
