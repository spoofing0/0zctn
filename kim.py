...
client = TelegramClient('kim_bot', API_ID, API_HASH)
...
def extract_largest_value_suit(cards_str):
    """GÜÇLENDİRİLMİŞ KART ANALİZİ - YENİ FORMATA UYGUN"""
    print(f"🔍 DEBUG - Kart analizi: {cards_str}")
    
    # YENİ FORMAT: "6(J♠️6♥️)" veya "2 (7♦️5♣️)" - boşlukları temizle
    cards_str_clean = cards_str.replace(' ', '')
    print(f"🔍 DEBUG - Temizlenmiş kart: {cards_str_clean}")
    ...
    # Kartları ayır - gelişmiş regex (emoji desteği ile)
...
def extract_game_info_from_message(text):
    """YENİ KANAL FORMATINA UYGUN OYUN BİLGİSİ ÇIKARMA - GÜNCELLENDİ"""
    game_info = {
    ...
    # YENİ KANAL REGEX PATTERN - YENİ FORMATA GÖRE GÜNCELLENDİ
    patterns = [
        r'[⏱⚠️]*\**№?(\d+)\**.*?(\d+\s*\([^)]+\)).*?(\d+\s*\([^)]+\))',  # ⏱**№653** formatı
        r'#N?(\d+).*?(\d+\s*\([^)]+\)).*?(\d+\s*\([^)]+\))',  # #N1063 formatı
        r'№(\d+).*?(\d+\s*\([^)]+\)).*?(\d+\s*\([^)]+\))',    # №653 formatı
    ]
    ...
    if game_match:
        ...
        # DEBUG: Pattern ve indikatör kontrolü
        print(f"🔍 DEBUG - Tespit edilen patternler: {game_info['patterns']}")
        print(f"🔍 DEBUG - Pattern gücü: {game_info['pattern_strength']}")
        print(f"🔍 DEBUG - Güçlü indikatör: {game_info['has_strong_indicator']}")
        print(f"🔍 DEBUG - Final kararı: {game_info['is_final']}")
    else:
        print(f"❌ DEBUG - Hiçbir regex eşleşmedi")
        # Debug için mesajın tamamını yazdır
        print(f"🔍 DEBUG - İşlenemeyen mesaj: {text}")
...
def should_send_signal(game_info):
    ...
    if performance_stats['consecutive_losses'] >= MAX_CONSECUTIVE_LOSSES:
        print("🔴 DEBUG - Maksimum ardışık kayıp limiti")
    ...
    if performance_stats['cooldown_mode']:
        if performance_stats['games_since_last_loss'] < COOLDOWN_AFTER_LOSS:
            print(f"⏳ DEBUG - Cooldown aktif: {performance_stats['games_since_last_loss']}/{COOLDOWN_AFTER_LOSS}")
    ...
    if not has_final_quality:
        print(f"❌ DEBUG - Final kalitesi yok. Güç: {game_info['pattern_strength']}, İndikatör: {game_info['has_strong_indicator']}")
        return False, "Final kalitesi yok"

    # 4. KART ANALİZİ - SON KONTROL
    signal_suit = extract_largest_value_suit(game_info['player_cards'])
    if not signal_suit:
        print("❌ DEBUG - Uygun kart yok")
...
async def send_optimized_signal(game_num, signal_suit, game_info):
    ...
    try:
        ...
        martingale_trackers[game_num] = {
            ...
            'expected_game_number_for_check': game_num,  # DÜZELTME: Aynı oyunu kontrol et
...
async def check_martingale_trackers():
    """DÜZELTİLMİŞ MARTINGALE TAKİP MEKANİZMASI"""
    global martingale_trackers, is_signal_active, performance_stats
    trackers_to_remove = []
    
    print(f"🔍 DEBUG - Martingale takipçileri kontrol ediliyor. Toplam {len(martingale_trackers)} takipçi var.")
    ...
    for signal_game_num, tracker_info in list(martingale_trackers.items()):
        ...
        print(f"🔍 DEBUG - Takipçi: Sinyal oyunu #{signal_game_num}, Kontrol edilen oyun: #{game_to_check}, Adım: {current_step}")
        
        # Oyun sonucu yoksa devam et
        if game_to_check not in game_results:
            print(f"❌ DEBUG - Oyun #{game_to_check} sonuçları henüz yok.")
            continue
            
        result_info = game_results.get(game_to_check)
        print(f"✅ DEBUG - Oyun #{game_to_check} sonuçları bulundu.")
        
        # Oyun sonucunda player kartları yoksa atla
        if not result_info.get('player_cards'):
            print(f"❌ DEBUG - Oyun #{game_to_check} player kartları yok.")
            continue

        player_cards_str = result_info['player_cards']
        print(f"🔍 DEBUG - Player kartları: {player_cards_str}")
        print(f"🔍 DEBUG - Aranan renk: {tracker_info['signal_suit']}")
        
        # DÜZELTME: Kartlarda renk arama - daha güvenli
        signal_won = False
        if tracker_info['signal_suit']:
            # Renk emojilerini kontrol et
            suit_emoji = tracker_info['signal_suit']
            signal_won = suit_emoji in player_cards_str
            print(f"🔍 DEBUG - Sinyal kazandı mı? {signal_won}")
        ...
        if signal_won:
            ...
            if current_step < MAX_MARTINGALE_STEPS:
                ...
                # MAKSİMUM KAYIP - SİSTEM DURDURMA
                performance_stats['max_martingale_steps_reached'] = max(
                    performance_stats['max_martingale_steps_reached'], 
    ...
    for game_num in trackers_to_remove:
        if game_num in martingale_trackers:
            ...
            print(f"🔧 DEBUG - Takipçi #{game_num} temizlendi")

    print(f"🔧 DEBUG - Kalan takipçiler: {len(martingale_trackers)}")

# YENİ KOMUT: Martingale durumunu kontrol et
@client.on(events.NewMessage(pattern='/trackers'))
async def trackers_command(event):
    """Aktif martingale takipçilerini göster"""
    ...
    for game_num, tracker in martingale_trackers.items():
        tracker_list.append(
            f"• #N{game_num} - {tracker['signal_suit']} "
            f"(Adım {tracker['step'] + 1}/{MAX_MARTINGALE_STEPS + 1})"
        )
    
    await event.reply("🔍 **Aktif Martingale Takipçileri:**\n" + "\n".join(tracker_list))

# YENİ KOMUT: Oyun sonuçlarını göster
@client.on(events.NewMessage(pattern='/results'))
async def results_command(event):
    """Son oyun sonuçlarını göster"""
    if not game_results:
        await event.reply("📊 **Henüz oyun sonucu yok**")
        return
    
    last_games = sorted(game_results.keys())[-10:]  # Son 10 oyun
    result_text = "📊 **Son Oyun Sonuçları:**\n"
    
    for game_num in last_games:
        game = game_results[game_num]
        result_text += f"• #N{game_num} - Player: {game['player_cards']}\n"
...
async def start_command(event):
    await event.reply(f"""
🤖 **Baccarat Bot - MARTINGALE DÜZELTİLMİŞ SÜRÜM** 🎰
**Kaynak Kanal:** {KANAL_KAYNAK_ID}

🆕 **DÜZELTMELER:**
• Martingale takip mekanizması iyileştirildi
• Oyun sonuçları doğru işleniyor
• Detaylı debug çıktıları eklendi

🚀 **OPTIMIZASYONLAR:**
• Sadece 4 en güçlü pattern
• 1-4 martingale'de yüksek kazanç hedefi

**Yeni Komutlar:**
/trackers - Aktif martingale'leri göster
/results - Son oyun sonuçlarını göster

**Komutlar:** 
/start, /stats, /status, /patterns, /active, /analysis, /reset
""")
...
🆕 **Yeni Format Desteği:** ⏱**№653** formatı
""")

# Mesaj İşleyici - GÜNCELLENDİ
...
async def handle_source_channel_message(event):
    ...
    print(f"✅ DEBUG - Oyun #{game_info['game_number']} sonuçları kaydedildi")
...
if __name__ == '__main__':
    print("🤖 Baccarat Bot - MARTINGALE DÜZELTİLMİŞ SÜRÜM Başlatılıyor...")
    print(f"🆕 YENİ KANAL: {KANAL_KAYNAK_ID}")
    print(f"🔧 MARTINGALE DÜZELTMELERİ: Aktif")
