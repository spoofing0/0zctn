...
client = TelegramClient('sago_bot', API_ID, API_HASH)
...
# YENİ KOMUT: Format testi için
@client.on(events.NewMessage(pattern='/test_format'))
async def test_format_command(event):
    """Yeni mesaj formatını test et"""
    test_message = "⏱**№653 6**(J♠️6♥️) -👉**2** (7♦️5♣️)"
    
    await event.reply(f"🧪 **Format Testi:**\n`{test_message}`")
    
    game_info = extract_game_info_from_message(test_message)
    
    result_text = f"""
🔍 **Test Sonuçları:**
• Oyun No: {game_info['game_number']}
• Player: {game_info['player_cards']}
• Banker: {game_info['banker_cards']}
• Patternler: {game_info['patterns']}
• Pattern Gücü: {game_info['pattern_strength']}
• İndikatör: {game_info['has_strong_indicator']}
• Final: {game_info['is_final']}
• Kart Sinyali: {extract_largest_value_suit(game_info['player_cards'])}
"""
...
async def start_command(event):
    await event.reply(f"""
🤖 **Baccarat Bot - GÜNCELLENMİŞ SÜRÜM** 🎰
**Kaynak Kanal:** {KANAL_KAYNAK_ID}

🆕 **YENİLİKLER:**
• Yeni mesaj formatı desteği: ⏱**№653** formatı
• Gelişmiş regex patternleri
...
/test_format - Yeni formatı test et
""")
...
# (/analysis, /status, /reset, /active, /martingale_stats, /max_step)
...
if __name__ == '__main__':
    print("🤖 Baccarat Bot - GÜNCELLENMİŞ SÜRÜM Başlatılıyor...")
    print(f"🆕 YENİ KANAL: {KANAL_KAYNAK_ID}")
    print(f"🎯 YENİ FORMAT: ⏱№653 formatı desteği eklendi")
    print(f"🔧 EN GÜÇLÜ 4 PATTERN: #C3_3, #C2_3, #C3_2, #R")
    print(f"📊 Min Güç: {MIN_PATTERN_STRENGTH}, Final Güç: {FINAL_MIN_STRENGTH}")
    print(f"🔍 ÖNERİ: Önce /test_format komutu ile formatı test edin!")
