...
async def update_martingale(current_game, player_cards_string):
    ...
    for signal_key, info in list(martingale_tracker.items()):
        ...
        if target_suit in player_cards_string:
            ...
            new_text = f"{current_time} | #N{bet_game} | {target_suit} - 7D | 10-BC-{type_display} | ✅ {STEP_EMOJIS[current_step]} | {win_message}"
            ...
            if next_step > MAX_MARTINGALE_STEP:
                ...
                new_text = f"{current_time} | #N{bet_game} | {target_suit} - 7D | 10-BC-{type_display} | ❌ | {loss_message}"
                try:
                    await client.edit_message(KANAL_HEDEF, info["msg_id"], new_text)
                ...
                new_text = f"{current_time} | #N{bet_game} | {target_suit} - 7D | 10-BC-{type_display} | 🔃 {STEP_EMOJIS[next_step]} | {waiting_message}"
                try:
                    await client.edit_message(KANAL_HEDEF, info["msg_id"], new_text)
...
async def send_signal(signal_game, signal_suit, signal_type):
    ...
    # GÜNCELLENMİŞ SİNYAL FORMATI: 14:30:15 | #N419 | ♣️ - 7D | 10-BC-Middle | 🔃 0️⃣ | Devam Ediyor…
    text = f"{current_time} | #N{signal_game} | {signal_suit} - 7D | 10-BC-{type_display} | 🔃 {STEP_EMOJIS[0]} | {waiting_message}"
    ...
    try:
        ...
        logger.info(f"🎯 YENİ SİNYAL: #N{signal_game} | {signal_suit} - 7D | 10-BC-{type_display}")
        return True
    except Exception as e:
