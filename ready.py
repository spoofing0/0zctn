...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/ready_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('ReadyBot')

# Config değişkenleri
API_ID = 29581698
API_HASH = '0caabd4263f1d4e5f753659a787c2e7d'
BOT_TOKEN = ''  # 🔑 BURAYA KENDİ BOT TOKEN'INI YAZ!
...
# CLIENT TANIMI
client = TelegramClient('/root/0zctn/ready_bot_session', API_ID, API_HASH)
...
# Excel dosyası
EXCEL_FILE = "/root/0zctn/ready_baccarat_data.xlsx"
...
🔴 SONUÇ: BEKLENİYOR..."""
        ...
        try:
            ...
            debug_log(f"✅ Hedef kanal bulundu: {target_entity.title}")
        except Exception as e:
            debug_log(f"❌ Hedef kanal bulunamadı: {e}")
            return
...
# ANA MESAJ İŞLEYİCİ - YENİ FORMAT İÇİN GÜNCELLENDİ
...
async def handle_source_channel_message(event):
    try:
        ...
        # YENİ FORMAT İÇİN GÜNCELLENMİŞ REGEX PATTERNLERİ
        # Örnek mesaj: "⏱**№190 👉4**(2♥️2♥️) -**4** (4♥️K♣️)..."
        
        # Oyun numarası için - "№190" formatı
        game_match = re.search(r'№(\d+)', text)
        if not game_match:
            game_match = re.search(r'#N(\d+)', text)  # Eski format için yedek
        
        # Player kartları için - "👉4**(2♥️2♥️)" formatı
        player_match = re.search(r'👉[^()]*\(([^)]+)\)', text)
        
        # Banker kartları için - "-**4** (4♥️K♣️)" formatı  
...
async def main():
    try:
        ...
        try:
            target_entity = await client.get_entity(KANAL_HEDEF)
            debug_log(f"✅ Hedef kanala erişim: {target_entity.title}")
        except Exception as e:
            debug_log(f"❌ Hedef kanala erişilemiyor: {e}")
            debug_log("🔧 Lütfen botun hedef kanala eklendiğinden emin olun!")

        try:
            source_entity = await client.get_entity(KANAL_KAYNAK_ID)
            debug_log(f"✅ Kaynak kanala erişim: {source_entity.title}")
        except Exception as e:
            debug_log(f"❌ Kaynak kanala erişilemiyor: {e}")
            debug_log("🔧 Lütfen botun kaynak kanala eklendiğinden emin olun!")
        
        debug_log("🤖 Bot aktif ve mesajları dinliyor...")
