...
client = TelegramClient('deep_bot', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
...
pattern_stats = {
    ...
    '🔄 ÇİFT TEKRAR': {'total': 0, 'wins': 0, 'losses': 0, 'profit': 0, 'avg_steps': 0},
    '🎲 RENK DENGESİZLİĞİ': {'total': 0, 'wins': 0, 'losses': 0, 'profit': 0, 'avg_steps': 0},
}
...
# ==================== YENİ PROFESYONEL PATTERN FONKSİYONLARI ====================

def hot_trend_pattern():
    """Aynı renk 4/5 son oyunda görülmüşse"""
    if len(color_trend) < 5: return None, None
    
    son_5 = color_trend[-5:]
    for renk in ['♠', '♥', '♦', '♣']:
        count = son_5.count(renk)
        if count >= 4:  # 5 oyundan 4'ü aynı renk
            return renk, f"🔥 SICAK TREND ({count}/5)"
    return None, None

def value_concentration_pattern(player_cards):
    """Aynı renkten 2 veya daha fazla yüksek değerli kart (9,8,7)"""
    try:
        player_kartlar = re.findall(r'(10|[A2-9TJQK])([♣♦♥♠])', player_cards)
        
        renk_yuksek_kart = {}
        for deger, renk in player_kartlar:
            if deger in ['9','8','7']:  # Yüksek değerli kartlar
...
def card_superiority_pattern(player_cards, banker_cards):
    """Oyuncu daha fazla kart almışsa ve toplam değer yüksekse"""
    try:
        player_kartlar = re.findall(r'(10|[A2-9TJQK])([♣♦♥♠])', player_cards)
        banker_kartlar = re.findall(r'(10|[A2-9TJQK])([♣♦♥♠])', banker_cards)
        
        if len(player_kartlar) > len(banker_kartlar):
            player_toplam = sum(get_baccarat_value(kart[0]) for kart in player_kartlar)
            if player_toplam >= 10:  # Yüksek toplam değer
                renk = extract_largest_value_suit(player_cards)
                if renk:
...
def momentum_shift_pattern():
    """Trendde ani değişim tespiti"""
    try:
        if len(color_trend) < 6: return None, None
        
        ilk_3 = color_trend[-6:-3]  # 3 oyun öncesi
        son_3 = color_trend[-3:]    # son 3 oyun
        
        ilk_dominant = max(set(ilk_3), key=ilk_3.count) if ilk_3 else None
        son_dominant = max(set(son_3), key=son_3.count) if son_3 else None
...
def double_repeat_pattern():
    """Aynı renk 2 kez üst üste, 3. için güçlü sinyal"""
    try:
        if len(color_trend) < 2: return None, None
        
        son_iki = color_trend[-2:]
        if son_iki[0] == son_iki[1]:
            return son_iki[0], "🔄 ÇİFT TEKRAR (2x Üst Üste)"
        return None, None
    except Exception as e:
...
def color_imbalance_pattern():
    """Belirli bir renk diğerlerine göre çok az çıkmışsa"""
    try:
        if len(color_trend) < 8: return None, None
        
        son_8 = color_trend[-8:]
        renk_frekans = {renk: son_8.count(renk) for renk in ['♠', '♥', '♦', '♣']}
        
        min_renk = min(renk_frekans, key=renk_frekans.get)
        min_count = renk_frekans[min_renk]
        
        if min_count <= 1:  # 8 oyunda 1 veya daha az
            return min_renk, f"🎲 RENK DENGESİZLİĞİ (Sadece {min_count}/8)"
        return None, None
    except Exception as e:
...
def analyze_professional_pattern(player_cards, banker_cards, game_number):
    """Tüm profesyonel patternleri kontrol et"""
    try:
...
async def handle_pro_pattern(event):
    ...
    analysis += "🔥 **ELITE PATTERNLER:**\n"
    analysis += "• 🔥 SICAK TREND (4/5 aynı renk)\n"
    analysis += "• 💎 DEĞER KONSANTRASYONU (2+ yüksek kart)\n"
    analysis += "• 🎯 KESİN KAZANÇ (P:8-9 vs B:0-5)\n"
    analysis += "• 📊 KART ÜSTÜNLÜĞÜ (daha fazla kart)\n"
    analysis += "• ⚡ ANI DEĞİŞİM (trend reversal)\n"
    analysis += "• 🔄 ÇİFT TEKRAR (2x üst üste)\n"
    analysis += "• 🎲 RENK DENGESİZLİĞİ (nadir renk)\n\n"
    analysis += "📈 **Sistem:** Tüm patternler entegre edildi!\n"
...
• /propattern - Profesyonel patternler
...
if __name__ == '__main__':
    ...
    print(f"🔥 PROFESYONEL PATTERN SİSTEMİ: AKTİF")
    print(f"🎯 7 Yeni Elite Pattern Entegre Edildi")
