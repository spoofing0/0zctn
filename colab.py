...
# ========================= MOTTO VE STRATEJİ HAVUZLARI =========================
SIGNAL_MOTTOS = [
    "⚠️ Risk yönetimine sadık kalınız.",
    "⚠️ Kasa planlamanızın dışına asla çıkmayınız.",
    "⚠️ Duygularınızla değil, sistemin matematiğiyle hareket edin.",
    "⚠️ Açgözlülük kasayı eritir; sadece sinyale odaklanın."
]

WIN_MOTTOS = [
    "🟢 Kasa büyüyecektir, disiplini bozmayın!",
    "🟢 Kâr cebe yakışır. Günlük hedefinize ulaştıysanız dinlenin.",
    "🟢 Sistem tıkır tıkır çalışıyor, heyecana kapılmadan plana sadık kalın."
]

LOSS_MOTTOS = [
    "🔴 Kayıplar oyunun parçasıdır. Stop-loss kurallarına uyun.",
    "🔴 Bugünlük bu kadar. Unutmayın, kasayı korumak da bir kazançtır.",
    "🔴 Hırsla masaya devam etmek kaybettirir. Botu bekleyin."
]

CANCEL_MOTTOS = [
    "⚪ Sabır en büyük sermayedir, doğru anı bekleyin.",
    "⚪ Risk almamak adına bu el pas geçilmiştir. Yeni tarama başlatıldı."
]
...
class MessageTemplates:
    ...
    def alert_message():
        motto = random.choice(SIGNAL_MOTTOS)
        return (
            f"🔍 **[ SİSTEM ALARMI: ANALİZ YAPILIYOR ]** 🔍\n"
            f"⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n"
            f"🚨 **Durum:** Masalarda Yüksek Yoğunluk Saptandı\n"
            f"📊 **Hacim:** Süit Gecikmeleri Sınır Bölgesinde\n"
            f"⏳ **Bekleme:** Sinyal Kriterleri Olgunlaşıyor...\n"
            f"⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n{motto}"
        )
...
def update_delays_and_analyze(game_num, player_cards_str):
    ...
    if best_val == (TRIGGER_DELAY_THRESHOLD - 1) and (best_val - second_val) >= 1:
        asyncio.create_task(client.send_message(TARGET_CHANNEL, MessageTemplates.alert_message()))
...
async def main():
    ...
    # Bağlantıyı asenkron olarak bekliyoruz (AWAIT EKLENDİ)
...
if __name__ == "__main__":
    # Colab'ın çalışan asenkron döngüsünü koruyarak tetikliyoruz
    try:
