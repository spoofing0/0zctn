    ...
    try:
        ...
        for game_num, tracker in list(martingale_trackers.items()):
            ...
            if expected_game in game_results:
                ...
                # Mesajı güncelle
                try:
                    original_text = tracker['message_obj'].text
                    new_text = original_text.replace("🔴 SONUÇ: BEKLENİYOR...", 
                                                   f"{result_emoji} SONUÇ: {result}")
                    await tracker['message_obj'].edit(new_text)
                except Exception as e:
                    logger.error(f"Mesaj güncelleme hatası: {e}")
                
                # Excel'de sonucu güncelle
                await update_excel_with_result(expected_game, result, profit)
                ...
                logger.info(f"✅ Sinyal #{game_num} sonuçlandı: {result}")
        ...
        logger.error(f"❌ Sonuç kontrol hatası: {e}")

async def update_excel_with_result(game_number, result, profit):
    ...
    try:
        ...
        for row in range(2, ws.max_row + 1):
            if ws.cell(row=row, column=1).value == game_number:
                ...
                result_cell = ws.cell(row=row, column=13, value=result)
                if result == "KAZANÇ": 
                ...
                ws.cell(row=row, column=14, value=profit)
                
                # Toplam Kâr'ı hesapla ve güncelle
                if row == 2:
                    ws.cell(row=row, column=15, value=profit)
                else:
                    prev_total = ws.cell(row=row-1, column=15).value or 0
                    ws.cell(row=row, column=15, value=prev_total + profit)
                
                break
                
        wb.save(EXCEL_FILE)
        logger.info(f"✅ Excel'de #{game_number} sonucu güncellendi: {result}")
        
    except Exception as e:
        logger.error(f"❌ Excel sonuç güncelleme hatası: {e}")
...
if __name__ == '__main__':
    ...
    # Önce eski session dosyalarını temizle
    session_files = ['/root/0zctn/royal_bot_session.session', 'royal_bot.session']
    for session_file in session_files:
        if os.path.exists(session_file):
            try:
                os.remove(session_file)
                print(f"✅ Eski session dosyası silindi: {session_file}")
            except Exception as e:
