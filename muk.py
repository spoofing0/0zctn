...
client = TelegramClient('baccarat_final_bot_v39', API_ID, API_HASH)
...
def get_last_card_suit(cards_str):
    """Oyuncunun son kartının rengini döndürür"""
    suits = suits_from_cards(cards_str)
    return suits[-1] if suits else None

def get_color_group(suit):
    """Rengin ait olduğu grubu döndürür"""
    if suit in red_group:
        return "RED"
    elif suit in black_group:
        return "BLACK"
    return None

def get_opposite_group_color(suit):
    """Verilen rengin zıt grubundan bir renk döndürür"""
    if suit in red_group:
        return "♣️"  # Siyah gruptan bir renk
    elif suit in black_group:
        return "♦️"  # Kırmızı gruptan bir renk
    return suit_flip.get(suit)
...
# -------------------------
# Sinyal Gönderme - GÜNCELLENMİŞ FORMAT
# -------------------------
async def send_signal(signal_game, signal_suit, signal_type, strategy_name="10-BC"):
    """Sinyal gönderme - YENİ FORMAT"""
    # Strateji ismini güncelle: 10-BC-First, 10-BC-Middle, 10-BC-Last
    strategy_with_type = f"10-BC-{signal_type.capitalize()}"
    
    # Sinyal key'ine strateji tipini de ekle
    signal_key = f"{signal_game}_{signal_suit}_{signal_type}_{strategy_with_type}"
    
    if signal_key in sent_signals:
        logger.debug(f"📨 Sinyal zaten gönderilmiş: {signal_key}")
        return False
    
    # YENİ FORMAT: #N1184 | ♠️ - 7D | 10-BC-First
    text = f"#N{signal_game} | {signal_suit} - 7D | {strategy_with_type}"
    ...
    try:
        ...
        martingale_tracker[signal_key] = {
            ...
            "strategy": strategy_with_type
        }
        logger.info(f"🎯 YENİ SİNYAL: #{signal_game} | {signal_suit} - 7D | {strategy_with_type}")
        return True
    except Exception as e:
...
# -------------------------
# 10-oyun geri sistemi (RENK GRUPLAMA İLE)
# -------------------------
async def check_10_game_back_system(current_game, current_player_cards):
    """10 oyun geri sistemi - RENK GRUPLAMA ile"""
    ...
    # İlk Kart Eşleşmesi (Renk Gruplama)
    current_first_suit = get_first_card_suit(current_player_cards)
    previous_first_suit = get_first_card_suit(previous_player_cards)
    
    if (current_first_suit and previous_first_suit and 
        get_color_group(current_first_suit) == get_color_group(previous_first_suit)):
        ...
        if signal_source_game in player_results:
            ...
            if signal_source_first_suit:
                # Zıt gruptan bir renk seç
                signal_suit = get_opposite_group_color(signal_source_first_suit)
                signal_game = get_next_game_number(previous_game_10, 11)
                success = await send_signal(signal_game, signal_suit, "first", "10-BC")
                if success:
                    logger.info(f"🔄 10-BC İlk kart: #{previous_game_10}-#{current_game} → #{signal_game} - {signal_suit} (Renk Gruplama)")

    # Orta Kart Eşleşmesi (Renk Gruplama)
    ...
    if (current_middle_suit and previous_middle_suit and 
        get_color_group(current_middle_suit) == get_color_group(previous_middle_suit)):
        signal_source_game = get_next_game_number(previous_game_10, 1)
        if signal_source_game in player_results:
            signal_source_middle_suit = get_middle_card_suit(player_results[signal_source_game])
            if signal_source_middle_suit:
                # Zıt gruptan bir renk seç
                signal_suit = get_opposite_group_color(signal_source_middle_suit)
                signal_game = get_next_game_number(previous_game_10, 11)
                success = await send_signal(signal_game, signal_suit, "middle", "10-BC")
                if success:
                    logger.info(f"🔄 10-BC Orta kart: #{previous_game_10}-#{current_game} → #{signal_game} - {signal_suit} (Renk Gruplama)")

    # Son Kart Eşleşmesi (Renk Gruplama)
    current_last_suit = get_last_card_suit(current_player_cards)
    previous_last_suit = get_last_card_suit(previous_player_cards)
    
    if (current_last_suit and previous_last_suit and 
        get_color_group(current_last_suit) == get_color_group(previous_last_suit)):
        signal_source_game = get_next_game_number(previous_game_10, 1)
        if signal_source_game in player_results:
            signal_source_last_suit = get_last_card_suit(player_results[signal_source_game])
            if signal_source_last_suit:
                # Zıt gruptan bir renk seç
                signal_suit = get_opposite_group_color(signal_source_last_suit)
                signal_game = get_next_game_number(previous_game_10, 11)
                success = await send_signal(signal_game, signal_suit, "last", "10-BC")
                if success:
                    logger.info(f"🔄 10-BC Son kart: #{previous_game_10}-#{current_game} → #{signal_game} - {signal_suit} (Renk Gruplama)")
...
async def handler(event):
    ...
    if player_has_arrow(text):
        logger.info(f"⏳ #N{game_number}: 3. kart bekleniyor - {player_cards}")
        return
...
async def main():
    logger.info("🎯 BACCARAT BOT v39 BAŞLATILIYOR...")
    logger.info("✅ GELİŞTİRİLMİŞ SİSTEM: 10-BC (Renk Gruplama + First/Middle/Last)")
    ...
    startup_msg = (
        ...
        f"🎯 Stratejiler: 10-BC (First/Middle/Last + Renk Gruplama)\n"
        f"⚡ Martingale: Step 0️⃣→{MAX_MARTINGALE_STEP}️⃣ ({MAX_MARTINGALE_STEP+1} adım)\n"
        f"🎴 Sadece Oyuncu Kartları: Evet\n"
        f"⏳ 3. Kart Bekleme: Aktif\n"
        f"🎨 Renk Gruplama: Kırmızı(♦️♥️) ↔ Siyah(♣️♠️)\n"
    ...
    logger.info("🟢 Bot çalışmaya hazır - 10-BC (Renk Gruplama) sistemi aktif...")
