...
📊 İstatistikler:
  ...
  Yükler     - Hedefin aldığı veri paketi sayısı
  Düşürülen  - Hedef tarafından reddedilen bağlantılar
...
if __name__ == '__main__':
    try:
        ...
        print(f"🎯 Hedef: {hedef} -> {hedef_ip}:{port}")
        ...
        while not durdur:
            ...
            sys.stdout.write(
                f'📈 İstatistikler | '
                f'🔗 Bağlantılar: {bağlantılar} | '
                f'📦 Yükler: {yükler} | '
                f'❌ Düşürülen: {düşürülen} | '
                f'🧵 Aktif Thread: {active_count()-1}          \r'
            )
            sys.stdout.flush()
