...
client = TelegramClient('baccarat_final_bot_v17', API_ID, API_HASH)
...
# -------------------------
# Martingale (DÜZELTİLMİŞ - BEKLENEN OYUN HESABI)
# -------------------------
async def update_martingale(current_game, player_cards_str):
    for bet_game, info in list(martingale_tracker.items()):
        ...
        # DÜZELTME: Beklenen oyun = sinyal oyunu + step + 1
        expected_game = get_next_game_number(bet_game, info["step"] + 1)
        ...
        if info["suit"] in player_cards_str:
            # KAZANILDI - ✅ işareti ve kazanılan step
            new_text = f"#N{bet_game} - {info['suit']}✅{type_tag} {step_emojis[info['step']]}"
            try:
                await client.edit_message(KANAL_HEDEF, info["msg_id"], new_text)
                print(f"[RESULT] ✅ #N{bet_game}: {new_text} (Step {info['step']}'de kazanıldı)")
...
async def send_signal(signal_game, signal_suit, signal_type):
    ...
    text = f"#N{signal_game} - {signal_suit}{step_emojis[0]}{type_tag}"
...
# -------------------------
async def main():
    print("🎯 Bakara Botu (v27) başlatılıyor... (DÜZELTİLMİŞ MARTINGALE)")
    await client.start()
