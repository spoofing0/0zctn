...
client = TelegramClient('yeni_bot', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
...
def get_daily_stats():
    ...
    return performance_stats['daily_stats'][today]
...
async def update_signal_message(tracker_info, result_type, current_step=None, result_details=None):
    try:
        ...
        # Mesaj silinmişse veya düzenlenemiyorsa tracker'ı temizle
        if "message ID is invalid" in str(e) or "can't do that operation" in str(e):
            print(f"🚮 Mesaj geçersiz, tracker temizleniyor: #{tracker_info['sent_game_number']}")
            if tracker_info['sent_game_number'] in martingale_trackers:
                del martingale_trackers[tracker_info['sent_game_number']]
...
# GÜNCELLENMİŞ extract_game_info_from_message FONKSİYONU
def extract_game_info_from_message(text):
    ...
    try:
        print(f"🔍 [EXTRACT] İşlenen metin: {text}")  # DEBUG
        
        # Daha esnek oyun numarası arama
        game_match = re.search(r'#N?(\d+)', text)  # #N123 veya sadece 123
        if game_match: 
            game_info['game_number'] = int(game_match.group(1))
            print(f"✅ Oyun numarası bulundu: {game_info['game_number']}")
        
        # Kartları bulma (daha esnek)
        # Player kartları: Player(....) veya Oyuncu(....)
        player_match = re.search(r'Player[^()]*\(([^)]+)\)', text) or re.search(r'Oyuncu[^()]*\(([^)]+)\)', text)
        if player_match: 
            game_info['player_cards'] = player_match.group(1)
            print(f"✅ Oyuncu kartları: {game_info['player_cards']}")
        else:
            # Alternatif: Parantez içindeki ilk kart seti
            all_parentheses = re.findall(r'\(([^)]+)\)', text)
            if len(all_parentheses) >= 1:
                game_info['player_cards'] = all_parentheses[0]
                print(f"✅ Oyuncu kartları (alternatif): {game_info['player_cards']}")
        
        # Banker kartları: Banker(....) 
        banker_match = re.search(r'Banker[^()]*\(([^)]+)\)', text) or re.search(r'Banker[^()]*\(([^)]+)\)', text)
        if banker_match: 
            game_info['banker_cards'] = banker_match.group(1)
            print(f"✅ Banker kartları: {game_info['banker_cards']}")
        else:
            # Alternatif: Parantez içindeki ikinci kart seti
            all_parentheses = re.findall(r'\(([^)]+)\)', text)
            if len(all_parentheses) >= 2:
                game_info['banker_cards'] = all_parentheses[1]
                print(f"✅ Banker kartları (alternatif): {game_info['banker_cards']}")
        ...
        for trigger_type, trigger_data in C2_3_TYPES.items():
            if trigger_type in text:
                ...
                print(f"✅ C2-3 tetikleyici: {trigger_type}")
                break
        
        # Final işareti (daha fazla pattern)
        final_indicators = ['✅', '🔰', '#X', 'FINAL', 'SONUÇ', 'SON', 'Bitti', 'bitti']
        if any(indicator in text for indicator in final_indicators):
            game_info['is_final'] = True
            print("✅ Final sonucu tespit edildi")
...
# QUANTUM VE MASTER SİSTEMLERİ (Geçici olarak normal hibrit ile aynı)
...
async def handle_source_channel_message(event):
    try:
        ...
        print(f"🔍 [DEBUG] Gelen Mesaj: {text}")  # DEBUG
        ...
        print(f"🔍 [DEBUG] Oyun Bilgisi: {game_info}")  # DEBUG
