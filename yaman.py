...
SISTEM_MODU = "super_hibrit"  # Varsayılan mod olarak Super Hibrit
GMT3 = pytz.timezone('Europe/Istanbul')
client = TelegramClient('yaman_bot', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
...
# 🔒 ÇİFT SİNYAL ÖNLEME SİSTEMİ
last_processed_game = 0  # Son işlenen oyun numarası
processing_lock = False  # İşlem kilidi
...
def hot_trend_pattern():
    """Aynı renk 4/6 son oyunda görülmüşse - DAHA DENGELİ"""
    try:
        ...
        for renk in ['♠', '♥', '♦', '♣']:
            ...
            # Eşik değerini 5'ten 4'e düşür ve çeşitlilik kontrolü ekle
            if count >= 4:  # 4/6 (%66.6) - DAHA DENGELİ
                # Aynı renk çok sık çıkmışsa, çeşitlilik için pas geç
                if len(color_trend) >= 10:
                    son_10 = color_trend[-10:]
                    if son_10.count(renk) >= 7:  # 10 oyunda 7'den fazla ise
                        print(f"🚫 {renk} rengi çok sık - çeşitlilik için pas")
                        return None, None
...
def card_superiority_pattern(player_cards, banker_cards):
    """Oyuncu daha fazla kart almışsa ve toplam değer yüksekse (10+)"""
    try:
        player_kartlar = re.findall(r'(10|[A2-9TJQK])([♣♦♥♠])', player_cards)
        banker_kartlar = re.findall(r'(10|[A2-9TJQK])([♣♦♥♠])', banker_cards)
        
        if len(player_kartlar) > len(banker_kartlar):  # +1 fark olmadan da çalışsın
            player_toplam = sum(get_baccarat_value(kart[0]) for kart in player_kartlar)
            if player_toplam >= 10:  # 10+ toplam yeterli
                renk = extract_largest_value_suit(player_cards)
                if renk:
...
def momentum_shift_pattern():
    """Trendde ani değişim tespiti - Daha dengeli eşik"""
    try:
...
def triple_repeat_pattern():
    ...
    try:
        ...
        print(f"❌ triple_repeat_pattern hatası: {e}")
        return None, None

def color_imbalance_pattern():
    """Bir renk 10 oyunda 2 veya daha az çıkmışsa - DAHA DENGELİ"""
    try:
        if len(color_trend) < 10: return None, None
        
        son_10 = color_trend[-10:]
        renk_frekans = {renk: son_10.count(renk) for renk in ['♠', '♥', '♦', '♣']}
        
        min_renk = min(renk_frekans, key=renk_frekans.get)
        min_count = renk_frekans[min_renk]
        
        if min_count <= 2:  # 10 oyunda 2 veya daha az
            return min_renk, f"🎲 RENK AÇIĞI ({min_count}/10)"
        return None, None
    except Exception as e:
...
def analyze_professional_pattern(player_cards, banker_cards, game_number):
    """Tüm profesyonel patternleri kontrol et - RENK ÇEŞİTLİLİĞİ EKLENDİ"""
    try:
        print("🎯 PROFESYONEL PATTERN analizi başlıyor...")
        
        # RENK ÇEŞİTLİLİĞİ KONTROLÜ - Son 5 sinyalde aynı renk 3'ten fazla ise, farklı renk tercih et
        son_renkler = []
        if len(color_trend) >= 5:
            son_5_renk = color_trend[-5:]
            for renk in ['♠', '♥', '♦', '♣']:
                if son_5_renk.count(renk) >= 3:  # Son 5'te 3 veya daha fazla aynı renk varsa
                    print(f"⚠️ {renk} rengi son 5 oyunda {son_5_renk.count(renk)} kez çıktı - Çeşitlilik için diğer renklere öncelik verilecek")
                    son_renkler.append(renk)
        ...
        # 3. Sıcak trend - Çeşitlilik kontrolü ile
        renk, sebep = hot_trend_pattern()
        if renk and renk not in son_renkler:  # Son zamanlarda çok çıkan renklerden değilse
            print(f"🎯 PROFESYONEL PATTERN: {sebep}")
            return renk, sebep
        ...
        # 6. Üçlü tekrar
        renk, sebep = triple_repeat_pattern()
        if renk: 
            print(f"🎯 PROFESYONEL PATTERN: {sebep}")
            return renk, sebep
        
        # 7. Renk açığı - Çeşitlilik için öncelikli
...
async def quantum_hibrit_sistemi(game_info):
    ...
    # DÜZELTME: Pattern isimlerini doğru göster
    sebep_metin = " + ".join([sebep for _, sebep, _, _ in pattern_sonuclari])
    if not sebep_metin:
        sebep_metin = "QUANTUM ANALIZ"
...
async def quantum_pro_sistemi(game_info):
    ...
    sebep_metin = " + ".join([sebep for _, sebep, _, _ in pattern_sonuclari])
    if not sebep_metin:
        sebep_metin = "QUANTUM PRO ANALIZ"
...
# 🔒 GÜNCELLENMİŞ ANA İŞLEYİCİ - ÇİFT SİNYAL SORUNU ÇÖZÜLDÜ
@client.on(events.NewMessage(chats=KANAL_KAYNAK_ID))
@client.on(events.MessageEdited(chats=KANAL_KAYNAK_ID))
async def handle_source_channel_message(event):
    try:
        global processing_lock, last_processed_game
        
        # 🔒 Eğer başka işlem devam ediyorsa bekle
        if processing_lock:
            print("🚫 İşlem kilitli, bekleniyor...")
            return
        ...
        # 🔒 AYNI OYUN TEKRAR İŞLENMESİN
        if game_info['game_number'] == last_processed_game:
            print(f"🚫 Aynı oyun tekrar işleniyor: #{game_info['game_number']} - Pas geçiliyor")
            processing_lock = False
            return
        ...
        if not is_signal_active:
            if game_info['is_final'] and game_info.get('is_c2_3'):
                ...
                if SISTEM_MODU == "normal_hibrit": 
                    ...
                    print(f"🚫 Bilinmeyen mod: {SISTEM_MODU}")
...
async def handle_mod_quantumpro(event):
    global SISTEM_MODU
    SISTEM_MODU = "quantum_pro"  # ✅ DÜZELTİLDİ
...
async def handle_temizle(event):
    ...
    global color_trend, recent_games, daily_signal_count, last_processed_game
    color_trend, recent_games, daily_signal_count, last_processed_game = [], [], 0, 0
...
🎯 **Özellikler:** Renk çeşitliliği + Pattern isimleri + Düzgün mod çalışması
🔒 **Güvenlik:** Çift sinyal koruması aktif
...
if __name__ == '__main__':
    ...
    print(f"🎯 Renk çeşitliliği optimizasyonu: AKTİF")
    print(f"🔒 Çift sinyal koruma sistemi: AKTİF")
    print(f"🔄 Tüm mod düzeltmeleri: TAMAMLANDI")
